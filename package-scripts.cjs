...
const customNps = (...cmds) => cmds.map(
  (cmd) => `nps -c ./package-scripts.cjs ${cmd}`
)
...
module.exports = {
  scripts: {
    ...
    format: {
      default: 'nps -c ./package-scripts.cjs lint.fix',
    },
    watch: {
    ...
    test: {
      ...
      setup: series(
        ...customNps('proxy.start', 'gitserver.start')
      ),
      teardown: series(
        ...customNps(
          'proxy.stop', 'gitserver.stop'
        )
      ),
      jest: process.env.CI
